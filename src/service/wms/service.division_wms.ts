...
class Division {
  ...
  deleteDivision = async (divisionCodes:{ company_code: string|undefined; div_code: string }[]) => {
    try {
      const response: IApiResponse<null> = await axiosServices.delete('api/wms/gm/division', 
        {
          data:divisionCodes
        }
      );
