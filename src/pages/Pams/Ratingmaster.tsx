  ...
  import AddRatingKpiForm from 'pages/Pams/AddRatingKpiForm';
  import useAuth from 'hooks/useAuth';
  // import { Tkkpimaster} from './type/KpiTypemaster-types';
  import pams from 'pages/Pams/pams_services';
  import {TRatingMaster } from './KpiTypemaster-types';


  const Ratingmasterpage = () => {
    //-------------- State ----------
    ...
    const [RatingmasterViewPopup, setRatingmasterViewPopup] = useState({
      action: { open: false }, 
      title: '',
      data: { existingData: null as any | null, isViewMode: false }
    });

    const [RatingmasterFormPopup, setRatingmasterFormPopup] = useState<TUniversalDialogProps>({
      action: { open: false, fullWidth: true, maxWidth: 'sm' },
      title: 'Add Rating Master ',
  ...
  const { data: kpiData, refetch: refetchKpiData } = useQuery({
    ...
    queryFn: async () => {
      ...
      const tableData = Array.isArray(response) ? response as TRatingMaster[] : [];
  ...
  // ===================== DELETE RATING =====================
  const handleDeleteRating = async (rowOriginal: TRatingMaster) => {
    if (!window.confirm('Are you sure you want to delete this Rating?')) return;

    await pams.proc_build_dynamic_del_pams({
      parameter: 'delete_rating',   // PL/SQL me case hona chahiye
      loginid: user?.loginid ?? '',
      code1: rowOriginal.RATING_CODE,   // RATING_CODE
      code2: user?.company_code ?? ''   // COMPANY_CODE
    });

    refetchKpiData(); // rating grid refresh
  };



    //-------------- Handlers ----------
    const toggleRatingmasterPopup = (refetchData?: boolean) => {
      setRatingmasterFormPopup((prev) => ({
        ...prev,
    ...
    const toggleRatingmasterViewPopup = () => {
      setRatingmasterViewPopup((prev) => ({
        ...prev,
    ...
    const handleActions = (actionType: string, rowOriginal: any) => {
      if (actionType === 'edit') {
        setRatingmasterFormPopup({
          ...RatingmasterFormPopup,
          title: 'Edit Rating Master',
          action: { ...RatingmasterFormPopup.action, open: true },
          data: { existingData: rowOriginal, isEditMode: true, isViewMode: false }
        });
      } else if (actionType === 'view') {
        setRatingmasterFormPopup({
          ...RatingmasterFormPopup,
          title: 'View Rating Master',
          action: { ...RatingmasterFormPopup.action, open: true },
          data: { existingData: rowOriginal, isEditMode: false, isViewMode: true }
        });
      } else if (actionType === 'delete') {
        handleDeleteRating(rowOriginal)
      }
    };
    ...
    return (
      <div className="flex flex-col space-y-2">
        ...
        <div className="flex justify-end space-x-2">
          ...
          <Button color="customBlue" startIcon={<PlusOutlined />} variant="contained" onClick={() => toggleRatingmasterPopup()}>
            Create Rating
          </Button>
        </div>
        ...
        {/* Form Dialog */}
        {RatingmasterFormPopup.action.open && (
          <UniversalDialog
            action={{ ...RatingmasterFormPopup.action }}
            onClose={() => toggleRatingmasterPopup(true)}
            title={RatingmasterFormPopup.title}
            hasPrimaryButton={false}
          >
            <AddRatingKpiForm
              onClose={() => toggleRatingmasterPopup(true)}
              isEditMode={RatingmasterFormPopup.data.isEditMode}
              isViewMode={RatingmasterFormPopup.data.isViewMode}
              existingData={RatingmasterFormPopup.data.existingData}
            />
          </UniversalDialog>
        )}

        {/* View Dialog */}
        {RatingmasterViewPopup.action.open && (
          <UniversalDialog
            action={{ ...RatingmasterViewPopup.action }}
            onClose={toggleRatingmasterViewPopup}
            title={RatingmasterViewPopup.title}
            hasPrimaryButton={false}
          >
            <AddRatingKpiForm
              onClose={toggleRatingmasterViewPopup}
              isEditMode={false}
              isViewMode={true}
              existingData={RatingmasterViewPopup.data.existingData}
            />
          </UniversalDialog>
        )}
      </div>
  ...
  export default Ratingmasterpage;
