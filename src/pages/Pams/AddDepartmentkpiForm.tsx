...
const AddDepartmentkpiForm = ({ onClose, isEditMode, existingData, isViewMode = false }: Props) => {
    ...
    onSubmit: async (values, { setSubmitting }) => {
      ...
      try {
        ...
        dispatch(
          showAlert({
            severity: 'success',
            message: isEditMode ? 'Department KPI updated successfully!' : 'Department KPI added successfully!',
            open: true
          })
        );
  ...
  useEffect(() => {
    ...
    if (existingData.KPI_CODE && !kpiMasterList.find((k) => k.kpi_type_code === existingData.KPI_CODE)) {
      setKpiMasterList((prev) => [
        ...prev,
        { kpi_code: existingData.KPI_CODE, kpi_type_code: existingData.KPI_CODE, kpi_desc: existingData.KPI_CODE }
      ]);
    }
      ...
      <Grid item xs={12} sm={6}>
        ...
        >
          ...
          {kpiMasterList.map((item) => (
            <MenuItem key={item.kpi_code} value={item.kpi_code}>
              {item.kpi_code} - {item.kpi_desc}
            </MenuItem>
          ))}
        </TextField>
      </Grid>
