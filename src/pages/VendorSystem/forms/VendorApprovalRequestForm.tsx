...
const VendorApprovalRequestForm: React.FC<VendorDetailsTabProps> = ({
  ...
  // preview dialog state for Bold report
  ...
  // Handler to call backend proc then open Bold report viewer
  const handleInvoicePrint = async () => {
    ...
    try {
      ...
      if (res?.success) {
        // open common Bold report viewer component and pass report id + params
      ...
      dispatch(showAlert({ open: true, message: error?.message || intl.formatMessage({ id: 'InvoicePrintError' }) || 'Error while printing invoice', severity: 'error' }));
    } finally {
  ...
  const handleSubmit = async (LAST_ACTION: string) => {
    ...
    try {
      ...
      console.log('inside form', requestNumber)
      if (requestNumber) {
      ...
      <Box sx={{ width: '100%' }}>
        ...
        <Paper elevation={1}>
            ...
            <Box sx={{ p: 2 }}>
              <Grid container spacing={2}>
                ...
                <Grid item xs={12} sm={3}>
                  {isEditMode ? (
                    ...
                    <Autocomplete
                      ...
                      renderInput={(params) => <TextField {...params} label={intl.formatMessage({ id: 'RefDocNo' }) || 'Ref Doc No'}
                        variant="outlined" />}
                    />
                  )}
                </Grid>
                ...
                <Grid item xs={12} sm={4}>
                  <TextField
                    fullWidth
                    label={intl.formatMessage({ id: 'InvoiceNo' }) || 'Ref Doc No'}
                    variant="outlined"
          ...
          {activeTab === 1 && (
            <Box sx={{ p: 2, maxWidth: '100%' }}>
              <VendorItemDetails vendorDetails={vendorDetails} hideReset={!showResetButton} requestNumber={requestData?.DOC_NO || ''} />
            </Box>
          )}
        </Paper>
        ...
        <Grid item xs={12} display="flex" justifyContent="space-between" alignItems="center" mt={3}>
          ...
          <ButtonGroup variant="outlined" size="medium" aria-label="Basic button group">
            {/* Print Invoice (left of Attach). Disabled when no DOC_NO available */}
        ...
        {previewReportPopup.action.open && (
          <UniversalDialog action={previewReportPopup.action} onClose={() => setPreviewReportPopup((p) => ({ ...p, action: { ...p.action, open: false } }))} title={previewReportPopup.title} hasPrimaryButton={false}>
            <WmsReportView
