...
const ManualRequestModal = ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      form.append('requested_by', selectedEmp?.full_name ?? selectedEmployee ?? 'Unknown');

      const response = await attendanceServiceInstance.createAttendanceRequest(form);
      if (response?.success) {
        message.success(`Request submitted. HR will review and approve it shortly.`);
        onClose();
      } else {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-[200]">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-auto overflow-hidden">
        ...
        {/* Header — deliberately muted to signal "exception flow" */}
        <div className="bg-amber-50 border-b border-amber-200 px-6 py-4 flex items-start justify-between">
          <div>
            ...
            <p className="text-amber-700 text-xs leading-snug">
              ...
              Your request will be reviewed by HR before being approved.
            </p>
          </div>
          <button
            onClick={onClose}
            className="text-amber-400 hover:text-amber-600 transition-colors ml-3 mt-0.5 flex-shrink-0"
          >
            <CloseOutlined />
          </button>
        </div>
        ...
        <div className="p-6 space-y-5">
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">
              <UserOutlined className="mr-1" />Employee
            </label>
            <Select
              showSearch
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">
              <CameraOutlined className="mr-1" />Photo
            </label>
          ...
          <div className="bg-gray-50 rounded-lg px-4 py-3 text-xs text-gray-500 border border-gray-100">
            ⏳ This request will be <strong>pending HR approval</strong>. It is not an instant check-in/out.
          </div>
...
// ─────────────────────────────────────────────────────────────────────────────
// MAIN COMPONENT — unchanged logic, manual request wired to failure states
// ─────────────────────────────────────────────────────────────────────────────
const AttendanceMarking = () => {
  ...
  const speak = useCallback(
    (text: string): Promise<void> => {
      return new Promise((resolve) => {
        if (!isVoiceEnabled || !speechSynthRef.current) { resolve(); return; }
        speechSynthRef.current.cancel();
  ...
  const getCurrentLocation = useCallback((): Promise<any> => {
    return new Promise((resolve, reject) => {
      if (USE_MOCK_LOCATION) {
        resolve({
          latitude: 19.0611456, longitude: 72.8727552, accuracy: 5,
          officeCheck: { isInOffice: true, office: GLOBAL_OFFICE_LOCATIONS[0], distance: 0 },
          locationType: 'office', timestamp: new Date().toISOString()
        });
        return;
      }
      ...
      navigator.geolocation.getCurrentPosition(
        async (position) => {
          ...
          resolve({ latitude, longitude, accuracy, officeCheck, locationType: officeCheck.isInOffice ? 'office' : 'remote', timestamp: new Date().toISOString() });
        },
        (error) => {
  ...
  const checkOfficeProximity = (lat: number, lng: number) => {
    ...
    for (const office of GLOBAL_OFFICE_LOCATIONS) {
      ...
      if (d < minDistance) { minDistance = d; nearestOffice = office; }
    }
    return { isInOffice: nearestOffice ? minDistance <= nearestOffice.radius : false, office: nearestOffice, distance: Math.round(minDistance) };
  };
  ...
  const detectFaceAngle = useCallback(async (imageSrc: string) => {
    try {
      ...
      catch { const CDN = 'https://cdn.jsdelivr.net/npm/face-api.js@0.22.2/weights'; await faceapi.nets.tinyFaceDetector.loadFromUri(CDN); await faceapi.nets.faceLandmark68Net.loadFromUri(CDN); }
      const img = new Image();
      ...
      const le = lm.getLeftEye(), re = lm.getRightEye(), nose = lm.getNose(), mouth = lm.getMouth();
      const ecX = (avg(le) + avg(re)) / 2, ecY = (avgY(le) + avgY(re)) / 2;
  ...
  const startAttendanceProcess = useCallback(async () => {
    if (attempts >= maxAttempts) { setStatus('error'); return; }
    try {
  ...
  const handleAutoCapture = useCallback(async () => {
    if (attempts >= maxAttempts) { setStatus('error'); return; }
    if (!userLocation) { handleCaptureError(); await speak('Location not available.'); return; }
    setStatus('capturing');
    await speak('Capturing now');
    await new Promise((r) => setTimeout(r, 300));
    const imageSrc = webcamRef.current?.getScreenshot();
    if (!imageSrc) { handleCaptureError(); return; }
    try {
  ...
  const handleAutoConfirm = useCallback(async (capturedPendingAttendance: any) => {
    ...
    try {
      ...
      if (confirmResponse.success) {
        ...
        setSuccessData({ ...capturedPendingAttendance, status: 'confirmed', employeeName, employeeCode: capturedPendingAttendance.recognized_employee?.code || capturedPendingAttendance.employee_code, action });
        setStatus('success');
      ...
      if (autoConfirmTimer) { clearInterval(autoConfirmTimer); setAutoConfirmTimer(null); }
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const handleAutoSubmit = useCallback(async (imageSrc: string) => {
    try {
        ...
        if (response.requires_confirmation) {
          ...
          const timer = setInterval(() => {
            setAutoConfirmCountdown((prev) => {
              if (prev <= 1) { clearInterval(timer); setAutoConfirmModal(false); handleAutoConfirm(response.data || response); return 0; }
              return prev - 1;
            });
          }, 1000);
          ...
          setSuccessData({ ...response, status: 'confirmed', employeeName, employeeCode: response.recognized_employee?.code || response.employee_code, action });
          setStatus('success');
          setIsModalOpen(false);
          await speak(`${employeeName}. ${action === 'check-in' ? 'Check in' : 'Check out'} done successfully.`);
          if (successTimerRef.current) clearTimeout(successTimerRef.current);
      ...
      if (isFaceError) {
        ...
        // After max attempts, show fallback — triggered from error state
      } else if (isNotFound) {
        // Employee not found → go straight to manual request
  ...
  const handleNotMe = useCallback(async () => {
    ...
    try {
      ...
      const cancelResponse = await attendanceServiceInstance.cancelAttendance({ uuid, actual_employee_code: 'NOT_RECOGNIZED', actual_employee_name: 'User Cancelled', reason: 'proxy_detected_by_user' });
      ...
      if (error.message?.includes('already confirmed')) { message.warning('Already confirmed automatically.'); }
      else { message.error('Cancellation failed.'); }
    }
  ...
  const renderLocationStatus = () => {
    ...
    if (locationStatus === 'error') return (
      <div className="flex items-center text-red-300 text-xs">
        <ExclamationCircleOutlined className="mr-1" />{locationError}
      </div>
    );
    return null;
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 py-4 px-3 sm:px-4 lg:px-6">
          ...
          <div className="flex flex-col lg:flex-row items-center justify-between gap-6">
            <div className="flex items-center space-x-4">
              <div className="relative">
                <div className="w-20 h-20 bg-gradient-to-br from-blue-600 via-purple-600 to-indigo-700 rounded-2xl flex items-center justify-center shadow-2xl">
                  <img src="/bayanatlogo.png" alt="Logo" className="w-14 h-14 object-contain filter brightness-0 invert" />
                </div>
                <div className="absolute -inset-2 bg-gradient-to-r from-blue-400 to-purple-500 rounded-2xl blur-lg opacity-30 -z-10" />
              </div>
              <div>
                <h1 className="text-3xl font-bold bg-gradient-to-r from-gray-800 to-gray-600 bg-clip-text text-transparent">Smart Attendance</h1>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <div className="bg-gradient-to-br from-blue-500 to-blue-600 text-white p-4 rounded-2xl shadow-lg min-w-[150px]">
                <div className="flex items-center space-x-2 mb-2"><CalendarOutlined className="text-blue-200 text-sm" /><span className="text-blue-200 text-xs font-medium">TODAY</span></div>
                <p className="text-lg font-bold">{new Date().toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' })}</p>
              </div>
        ...
        <div className="bg-white/90 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/20 overflow-hidden">
          <div className="p-8 lg:p-12">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
              <button onClick={() => handleActionClick('check-in')} className="group relative bg-gradient-to-br from-green-50 to-emerald-50 border-2 border-green-200/50 rounded-3xl p-8 transition-all duration-500 hover:scale-105 hover:shadow-2xl hover:border-green-300">
                <div className="relative z-10 text-center">
                  <div className="relative inline-block mb-6">
                    <div className="w-24 h-24 bg-gradient-to-br from-green-500 to-emerald-600 rounded-2xl flex items-center justify-center group-hover:scale-110 transition-transform duration-500 shadow-2xl">
                      <ArrowRightOutlined className="text-white text-2xl" />
                    </div>
                  </div>
                  <h3 className="text-2xl font-bold text-gray-800 mb-3">Check In</h3>
                  <p className="text-green-600 font-medium text-lg mb-4">Start your productive day</p>
                  <div className="flex items-center justify-center space-x-2 text-green-500">
                    <EnvironmentOutlined className="text-sm" /><span className="text-sm font-medium">Location Verified</span>
                  </div>
                </div>
              </button>

              <button onClick={() => handleActionClick('check-out')} className="group relative bg-gradient-to-br from-blue-50 to-indigo-50 border-2 border-blue-200/50 rounded-3xl p-8 transition-all duration-500 hover:scale-105 hover:shadow-2xl hover:border-blue-300">
                <div className="relative z-10 text-center">
                  <div className="relative inline-block mb-6">
                    <div className="w-24 h-24 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-2xl flex items-center justify-center group-hover:scale-110 transition-transform duration-500 shadow-2xl">
                      <ArrowLeftOutlined className="text-white text-2xl" />
                    </div>
                  </div>
                  <h3 className="text-2xl font-bold text-gray-800 mb-3">Check Out</h3>
                  <p className="text-blue-600 font-medium text-lg mb-4">End your work day</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[
                { icon: <ScanOutlined className="text-white text-lg" />, from: 'from-purple-500', to: 'to-pink-500', bg: 'from-purple-50 to-pink-50', border: 'border-purple-200/50', title: 'Auto Recognition', desc: 'AI-powered face recognition with auto-confirm' },
                { icon: <EnvironmentOutlined className="text-white text-lg" />, from: 'from-blue-500', to: 'to-cyan-500', bg: 'from-blue-50 to-cyan-50', border: 'border-blue-200/50', title: 'Location Smart', desc: 'Automatic office/remote location detection' },
                { icon: <UserOutlined className="text-white text-lg" />, from: 'from-green-500', to: 'to-emerald-500', bg: 'from-green-50 to-emerald-50', border: 'border-green-200/50', title: 'Voice Guided', desc: 'Step-by-step voice instructions' },
              ].map((f) => (
                <div key={f.title} className={`bg-gradient-to-br ${f.bg} rounded-2xl p-6 border ${f.border} text-center`}>
                  <div className={`w-12 h-12 bg-gradient-to-br ${f.from} ${f.to} rounded-xl flex items-center justify-center mx-auto mb-4`}>{f.icon}</div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-2 sm:p-4 z-50">
          <div className="bg-white rounded-2xl sm:rounded-3xl shadow-2xl w-full max-w-2xl mx-2 sm:mx-4 overflow-hidden max-h-[90vh] overflow-y-auto">
            <div className="bg-gradient-to-r from-blue-600 to-purple-700 py-3 sm:py-4 px-4 sm:px-6 flex justify-between items-center sticky top-0 z-10">
              <div className="flex items-center flex-wrap gap-2">
                ...
                {attempts > 0 && <span className="px-2 py-1 bg-blue-500/30 rounded-full text-blue-200 text-xs">Attempt {attempts}/{maxAttempts}</span>}
              </div>
              <button onClick={handleCloseModal} className="text-white hover:text-blue-200 p-1 rounded-full hover:bg-white/10 flex-shrink-0" disabled={isSubmitting}>
              ...
              {status === 'error' ? (
                <div className="text-center py-8 sm:py-10">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 justify-center">
                    <button onClick={handleRetry} className="inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-gradient-to-r from-green-500 to-green-600 text-white rounded-xl font-semibold text-sm transition-all hover:scale-105 shadow-md">
                      <RedoOutlined /> Try Again
                    </button>
                    {/* ← FALLBACK TRIGGER: only visible after all attempts exhausted */}
                    <button
                      onClick={() => {
                ...
                <div className="space-y-3 sm:space-y-4">
                  <div className="relative rounded-xl overflow-hidden bg-gray-100 border-2 border-gray-200" style={{ height: getCameraContainerHeight() }}>
                    <Webcam audio={false} ref={webcamRef} screenshotFormat="image/jpeg" videoConstraints={getVideoConstraints()} style={{ width: '100%', height: '100%', display: 'block', objectFit: 'cover' }} screenshotQuality={0.92} mirrored />
                  ...
                  <div className="bg-gradient-to-r from-blue-50 to-purple-50 rounded-xl p-3 border border-blue-200">
                    ...
                    {status === 'idle' && (
                      <div className="text-blue-600 text-xs text-center grid grid-cols-2 gap-1">
                        <div className="flex items-center justify-center"><BulbOutlined className="mr-1" /><span>Good lighting</span></div>
                        <div className="flex items-center justify-center"><WarningOutlined className="mr-1" /><span>No sunglasses</span></div>
                        <div className="flex items-center justify-center"><UserOutlined className="mr-1" /><span>Look at camera</span></div>
                        <div className="flex items-center justify-center"><ScanOutlined className="mr-1" /><span>Fill the frame</span></div>
                      </div>
                    )}
                  </div>
                </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-auto overflow-hidden">
            ...
            <div className="p-6">
              <div className="text-center mb-6">
                <img src={pendingAttendance.recognized_employee?.image || '/default-avatar.png'} alt="Recognized" className="w-20 h-20 rounded-2xl object-cover border-4 border-blue-400 shadow-lg mx-auto mb-4" />
                <h4 className="text-xl font-bold text-gray-800 mb-2">{pendingAttendance.recognized_employee?.name || pendingAttendance.employee_name}</h4>
                <p className="text-gray-600 text-sm">{pendingAttendance.recognized_employee?.code || pendingAttendance.employee_code}</p>
                {userLocation && (
                  <div className="bg-blue-50 rounded-lg p-3 mt-3">
                    <div className="flex items-center justify-center text-blue-700 text-xs mb-1"><EnvironmentOutlined className="mr-1" />{userLocation.officeCheck.isInOffice ? '🏢 Office' : '🌍 Remote'}</div>
                    <p className="text-blue-600 text-xs">{userLocation.officeCheck.distance}m from office center</p>
                  </div>
                )}
              </div>
              <button onClick={handleNotMe} className="w-full bg-gradient-to-r from-red-500 to-pink-600 hover:from-red-600 hover:to-pink-700 text-white py-3 px-6 rounded-xl font-semibold text-base transition-all hover:scale-105 shadow-lg">
                ❌ It's Not Me — Cancel
              </button>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 p-6">
            <div className="text-center">
              ...
              <h4 className="text-2xl font-bold text-gray-800 mb-3">{successData.action === 'check-in' ? 'Checked In! 🎉' : 'Checked Out! 👋'}</h4>
              <p className="text-gray-600 text-base mb-4">Attendance recorded for <strong className="text-gray-800">{successData.employeeName}</strong></p>
              <p className="text-gray-500 text-sm mb-2">Code: <strong className="text-gray-700">{successData.employeeCode}</strong></p>
              {userLocation && (
                <div className="bg-blue-50 rounded-xl p-3 mb-4">
                  <div className="flex items-center justify-center text-blue-700 text-xs mb-1"><EnvironmentOutlined className="mr-1" />{userLocation.officeCheck.isInOffice ? '🏢 Office Location' : '🌍 Remote Location'}</div>
              ...
              <button onClick={handleSuccessClose} className="bg-gradient-to-r from-blue-500 to-purple-600 text-white py-2 px-6 rounded-xl font-semibold transition-all hover:scale-105 shadow-lg">Got It!</button>
            </div>
          </div>
        </div>
      )}

      {/* Manual Request Fallback Modal — only rendered after failure */}
      {showManualRequest && (
        <ManualRequestModal
